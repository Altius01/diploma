...
def main(start_step = 0):
  ...
  t = start_step * 0.5*(1/256)**2 / 100

  global T

  steps = 10
  T = t + steps*(0.5*(1/64)**2)
  i = start_step
  while t < T:
    # dT = 0.5*(1/256)**2
    dT = 0.5*(1/64)**2
...
if __name__ == "__main__":
    main()
    data_service = DataService(str(date.today()), scalar_shape, vec_shape)
    data_service.read_data(0, (u, B_arr, rho, p))

    # plt.plot(list(range(len(u[0, 0, GHOSTS:SHAPE[1]-GHOSTS, 0]))), u[0, 0, GHOSTS:SHAPE[1]-GHOSTS, 0])
    # plt.show()

    # data_service.read_data(100, (u, B_arr, rho, p))
