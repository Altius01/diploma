...
from new_src.problem.problem import LegacyCompatibleProblem2D, Problem2D, ProblemConfig

from new_src.common.types import *
from new_src.common.matrix_ops import *
...
class TiSystem:
    def __init__(self, config, data_path="", arch=ti.cpu):
        ...
        Logger.log(
            f"   CFL: {self.CFL}, LES_Model: {self.les_model}, Ideal: {self.ideal}, Hall: {self.hall}"
        )
        Logger.log(
            f"   Re: {self.Re}, Rem: {self.Rem}, delta_hall: {self.delta_hall}, Ma: {self.Ma}, Ms: {self.Ms}, gamma: {self.gamma}"
        )
        ...
        self.data_service = DataService(
            dir_name=data_path, rw_energy=self.config.rewrite_energy
        )

        self.u = [
            ti.Vector.field(n=3, dtype=double, shape=self.config.shape)
            for i in range(self.rk_steps)
        ]
        self.B = [
            ti.Vector.field(n=3, dtype=double, shape=self.config.shape)
            for i in range(self.rk_steps)
        ...
        self.fv_computer = LegacyCompatibleProblem2D(
            ProblemConfig(
                self.gamma,
                self.Re,
                self.Ms,
    ...
    def initials_ghosts(self):
        ...
        if self.div_cleaning == True:
            self.update_B_staggered_call(0)
    ...
    def sum_fields_u(
        ...
        for idx in ti.grouped(a):
            c[idx] = (c1 * a[idx] * rho_old[idx] + c2 * b[idx]) / (
                rho_new[idx]
            ) + c3 * c[idx]
    ...
    def FV_step(self, dT):
        ...
        # if self.debug_fv_step:
        #     print(
        #         f"    Les coefs: C:{self.fv_computer.C}, Y:{self.fv_computer.Y}, D: {self.fv_computer.D}"
        #     )
